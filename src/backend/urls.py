...
urlpatterns = [
    ...
    url(r'^reset_password/(?P<pk>[0-9]+)/(?P<token>(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?)$',
        views.reset_password, name='reset_password'),
    url(r'^request_reset_password/(?P<email>.*@([a-zA-Z0-9\/\+]*\.)?utwente\.nl)/?$', views.request_reset_password,
